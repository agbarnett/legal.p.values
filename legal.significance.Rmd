...
library(stringr)
## a) get the meta-data
meta = read.table('meta.case.data.txt', sep='\t', header=T, stringsAsFactors = F, quote = "", comment.char = '')
...
pander(tab, emphasize.rownames = F)
...
pander(tab, style='simple', emphasize.rownames=F)
```
...
pander(bugs.summary, style='simple', emphasize.rownames=F)
```
...
merged = merge(subset(data, select=-type), 
               subset(meta, select=c('case','type','brief','court')), by='case', all.x=T)
...
merged$type.other[merged$type.other%in%top.ten.type ==F] = 'Other'
# with at least 10 interpretations
#at.least.10 =  names(which(table(merged$type)>=10))
#merged$type.other[merged$type.other%in%at.least.10 ==F] = 'Other'
# table
...
merged$brief.other[merged$brief.other%in%top.ten.brief ==F] = 'Other'
# with at least 10 interpretations
#at.least.10 =  names(which(table(merged$brief)>=10))
#merged$brief.other[merged$brief.other%in%at.least.10 ==F] = 'Other'
# table
